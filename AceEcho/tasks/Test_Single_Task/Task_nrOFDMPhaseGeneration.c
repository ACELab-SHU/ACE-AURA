...
int Task_nrOFDMPhaseGeneration(short_struct subCarrierSpace, short_struct symbolNum, int_struct freq0) {
  ...
  if(symbolPhases<1){
    __v4096i8 Phase_cos1;
    __v4096i8 Phase_sin1;
    vclaim(Phase_cos1);
    vclaim(Phase_sin1);
    vbrdcst(Phase_cos1, cos_result, MASKREAD_OFF, N_FFT);
    vbrdcst(Phase_sin1, sin_result, MASKREAD_OFF, N_FFT);
    Phase_cos = vsadd(Phase_cos1, 0, MASKREAD_OFF, N_FFT);
  }
  else
  {
    __v4096i8 Phase_cos1;
